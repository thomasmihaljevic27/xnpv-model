"""Independent reproduction and targeted probes of goalie candidate af61f6c."""
import argparse
import json
import os
...
CANDIDATE = ROOT/'50_REBUILD/output/goalie_price_repair_review'
...
if args.mode=='setup':
    ...
    try:
        ...
        P.fit_rolling=fit
    response=P.price_response
    P.price_response=lambda coef,feats,cap,goalie,rfa: P.dollars_per_win(coef,feats,cap,'g_x_war' if goalie else None)
    try:
        RC.c33(None)
        results['partial instead of whole path']='MISSED'
    except AssertionError:
        results['partial instead of whole path']='caught'
    finally:
        P.price_response=response
    print(json.dumps(results,indent=2))
    assert all(v=='caught' for v in results.values())
    (ROOT/'50_REBUILD/output/goalie_price_repair_mutations.json').write_text(json.dumps(results,indent=2))
elif args.mode=='run':
    import run_goalie_price_line as P
    prepare=P.prep_pooled
    def capture(sk,go):
        d=prepare(sk,go)
        d.to_csv(ROOT/'50_REBUILD/output/goalie_price_repair_sample.csv',index=False)
        return d
    ...
    P.main()
    (ROOT/'50_REBUILD/output/goalie_price_repair_fits.json').write_text(json.dumps(fits,indent=2))
else:
    import run_goalie_price_line as P
    d=pd.read_csv(ROOT/'50_REBUILD/output/goalie_price_repair_sample.csv',parse_dates=['signed'])
    d['cut']=d.signed.dt.to_period('Q').dt.start_time
    fits=json.loads((ROOT/'50_REBUILD/output/goalie_price_repair_fits.json').read_text())
    ...
    scored.to_csv(ROOT/'50_REBUILD/output/goalie_price_repair_independent_scores.csv',index=False)
    ...
    for rfa in [0,1]:
        ...
        for goalie in [0,1]:
            ...
            assert abs(P.price_response(np.asarray(last['coef']),P.POOLED,cap,goalie,rfa)-expected)<1e-6
    ...
    # Give every relevant coefficient a nonzero, distinct value so omitting
    # the RFA or first-year term cannot pass through a zero-valued fixture.
    for features in [P.POOLED,P.LEVEL_ONLY]:
        coefficients=np.zeros(1+len(features))
        for feature,value in [('war_per_season',.01),('war_year1',.02),('rfa_x_war',-.003),('g_x_war',.004)]:
            if feature in features:
                coefficients[1+features.index(feature)]=value
        for goalie in [0,1]:
            for rfa in [0,1]:
                expected=(.03-.003*rfa+(.004*goalie if 'g_x_war' in features else 0))*cap
                assert abs(P.price_response(coefficients,features,cap,goalie,rfa)-expected)<1e-6
    ...
    (ROOT/'50_REBUILD/output/goalie_price_repair_independent_audit.json').write_text(json.dumps(result,indent=2))
