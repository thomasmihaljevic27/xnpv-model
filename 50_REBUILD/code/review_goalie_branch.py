"""Independent reproduction and targeted probes of goalie candidate 08731eb."""
import argparse
import json
import os
...
CANDIDATE = ROOT/'50_REBUILD/output/goalie_review'
...
parser.add_argument('mode', choices=['setup','run','checks','audit','rfa'])
...
if args.mode=='setup':
    ...
    base=data[data.rule==B.ProductionRule.name]
    comparisons={}
    ...
    actual.to_csv(ROOT/'50_REBUILD/output/goalie_actual_projector_scored.csv',index=False)
    ...
    (ROOT/'50_REBUILD/output/goalie_review_audit.json').write_text(json.dumps(result,indent=2))
