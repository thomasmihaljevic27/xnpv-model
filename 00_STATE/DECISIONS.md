...
- **2026-09-14b (MANIFEST.csv reconciled; a lost `.docx` extension restored):** Thomas asked for the two manifest problems flagged in the 2026-09-14 entry to be fixed. Audited the file against `git ls-files` instead of fixing the two by hand, which found more. Scope comes from PROJECT_STATE's MANIFEST.csv section: every git-tracked file under `20_CODE`, `00_STATE`, `10_SOURCE`, and `40_DOCS`, plus the `30_OUTPUT` working set and the game-log DB, so repo-root files and `90_ARCHIVE` are out of scope and account for 11 of 26 apparent gaps. **In scope: 9 stale rows, 15 missing rows, 5 drifted hashes.** Stale were the retired `00_Immediate_Work.md`, four top-level `Doc_N` explainers, `Docs_3_4_5_Review_Notes.md`, and the four `Supervisor_Drafts/Doc_N` copies superseded by the human-titled documents. Missing were 9 session files (2026-09-09b through 2026-09-12), `Design Questions.txt`, the four human-titled `Supervisor_Drafts` documents, and `Doc_3_Player_Pillar_II_2.docx`. Result 131 rows to 137, re-audited clean on stale rows, in-scope gaps, hash mismatches, duplicate paths, and field counts. **One real defect rather than bookkeeping: `40_DOCS/Doc_3_Player_Pillar_II_2` had lost its `.docx` extension.** `sessions/2026-09-11c.md` records it as `Doc_3_Player_Pillar_II_2.docx`, `file` reports Microsoft Word 2007+, and it opens as a valid zip; it entered the repository unextensioned in the 2026-09-13 19:54 sync commit. Restored with `git mv` as a repair, the no-rename rule covering `20_CODE` and `30_OUTPUT` where names are load-bearing in code. **A near-miss worth recording, because it is the project's own size-is-not-an-integrity-check rule firing in practice:** the first pass refreshed 56 rows, since `size_bytes` disagrees with disk on most text files while the hashes match. The recorded sizes were taken on a CRLF machine and this container is LF, so `contract_npv.py` reads 42,810 bytes in the manifest against 42,026 here on an identical blob, and writing the LF sizes would have made each row mismatch on Thomas's machine. Redone on hash only with recorded sizes untouched. **Also corrected an earlier edit from the same day:** the six rows created in the 2026-09-14 entry were set `verified_locally=no`, but the protocol defines that field for `channel = git` rows as yes whenever the working tree is clean, since git is the audit for those rows; set to yes. The MANIFEST.csv self-row's note now states that the hash column cannot be self-referential and is not audited. **Left for Thomas, both judgment calls:** `90_ARCHIVE/` is gitignored yet `90_ARCHIVE/2026-09-12/00_Immediate_Work.md` is tracked, because an ignore rule does not untrack a committed file, and `git rm --cached` would drop it from a fresh clone; and `size_bytes` is now mixed-platform, harmless under a protocol that does not audit on size but no longer comparable row to row. No code, model, or decision change.

- **2026-09-14 (cap inflation in the NPV; a new g/rho confound; discount-heterogeneity sequence):** Question-and-answer session, no code run and no model change (cloud container, no `.env`, empty `30_OUTPUT/`; every figure read from `20_CODE/` or computed analytically inline). Confirmed from the code that the model does hold value in cap-share terms against a flat nominal cap hit: `cap_path()` grows the ceiling at `CAP_GROWTH` (`skater_forward_projection.py` 381-386), value prices off it at line 727, cost is the per-season cap hit (line 523, not assumed flat), and `contract_npv.py` discounts at the same constant (197, 426). **Corrected two premises.** (1) The discount rate does not erode the benefit of term: g cancels on the value side (D24) and the surviving `cost / (1+g)^k` IS the declining cap share, so there is no pure time discount in cap-share space and term is rewarded monotonically (+$0.23M / +$1.37M / +$6.16M at two, four, and eight years, g=3%, survival and aging aside). The only brakes are the exit hazard and the aging curve. (2) Long term does not lower AAV in this sample: `term_premium_test.py` carries `c_len = 0.00859642`, or +$0.82M of annual cap hit per extra year, so long deals show negative NPV because clubs pay a term premium Stage 2 found buys no production, not because discounting penalises length. **NEW STANDING FLAG, and the substantive output of the session: g and a recovered discount rate are not separately identified.** Because the cost-erosion channel runs at exactly g, the same eight-year deal carries +$6.16M at g=3%, +$9.71M at 5%, and +$13.77M at 7.6%, against realized ceiling growth of 7.6%/yr from 2023-24 to 2026-27. Two consequences: the 3% path compresses the reward for length, which is the channel the long-contract negative-NPV finding runs through, making part of that finding a candidate g artifact that Stage 2 did not test; and under-crediting far-dated cap relief is indistinguishable from clubs over-discounting the future, so any recovery of club or era rates must sweep g. The existing published-ceiling flag records the same constant from the level side; this is the term-gradient side. **New sequence document `04_Discount_Heterogeneity_Sequence.md`**, in response to Thomas's question about recovering rate differences by era or build timeline. Key conclusions: the sequencing inverts, since identifying variation is in the 654 of 946 pick trades (69.1%) and not the 205 clean player-only ones, so this is gated on Phase 3 in full; the shelved pure-pick finding killed a level, not a difference, because a corner solution still yields an inequality and common censoring differences out (bounds, not points, as that item already recorded, with Spearman 0.735 the evidence the bounds will bind); club-specific rates are out of scope at roughly 59 trade-sides per club against a modelled dependent variable, though era effects and a club-state covariate are estimable; the covariate must be points percentage and games remaining at the trade date, reconstructible from the `games` table (11,870 games 2017-18 to 2025-26) and never final standing; the bargaining restriction must be chosen in writing first, since one price carries two rates; and the long-horizon projection needs a Game Value check at k=4..8 before any figure is quoted, because `_proj_sd_war` holds the k=3 spread flat beyond k=3 and the 2026-09-13b star over-projection is a horizon-correlated bias in exactly that population. D11, D15, and D17 untouched and still locked. Updated: STANDING_FLAGS (new entry at the top), WORK_QUEUE (Phase 2 second arm on the power-analysis re-run, new Phase 4 item 4e), MANIFEST row, `sessions/2026-09-14.md`. Open: the g sweep at {3%, 5%, 7.6%} on the spine, which needs a machine with the paths.
...
### Change log, 2026-09-24 (sustained-quality review)
...
check 47 extended, check 48 new; suite 48/48. `run_skater_dollar_scoring.py` v1.1 (`price_and_simulate`);
